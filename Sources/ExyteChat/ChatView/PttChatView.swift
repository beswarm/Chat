...
class  FakeNetworkMonitor: ObservableObject {
    var isConnected: Bool = false
}

public struct PttChatView<MessageContent: View, InputViewContent: View>: View {
    ...
    /// - pass attachment to this closure to use PttChatView's fullscreen media viewer
...
public extension PttChatView {
    
    func chatType(_ type: ChatType) -> PttChatView {
        var view = self
        view.type = type
        return view
    }
    
    func showDateHeaders(showDateHeaders: Bool) -> PttChatView {
        var view = self
        view.showDateHeaders = showDateHeaders
        return view
    }
    
    func avatarSize(avatarSize: CGFloat) -> PttChatView {
        var view = self
        view.avatarSize = avatarSize
        return view
    }
    
    func messageUseMarkdown(messageUseMarkdown: Bool) -> PttChatView {
        var view = self
        view.messageUseMarkdown = messageUseMarkdown
        return view
    }
    
    func showMessageMenuOnLongPress(_ show: Bool) -> PttChatView {
        var view = self
        view.showMessageMenuOnLongPress = show
        return view
    }
    
    func tapAvatarClosure(_ closure: @escaping TapAvatarClosure) -> PttChatView {
        var view = self
        view.tapAvatarClosure = closure
        return view
    }
    
    func assetsPickerLimit(assetsPickerLimit: Int) -> PttChatView {
        var view = self
    ...
    func setMediaPickerSelectionParameters(_ params: MediaPickerParameters) -> PttChatView {
        var view = self
        view.mediaPickerSelectionParameters = params
        return view
    }
    
    func orientationHandler(orientationHandler: @escaping MediaPickerOrientationHandler) -> PttChatView {
        var view = self
    ...
    func enableLoadMore(offset: Int = 0, handler: @escaping ChatPaginationClosure) -> PttChatView {
        var view = self
